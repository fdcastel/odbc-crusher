...
// D60: one implementation, in core/guarded_buffer.hpp. This forwards, so
// the probes keep calling it unqualified the way A3 left them.
BoundedString TestBase::bounded_string(const char* buf, size_t capacity,
                                       SQLLEN reported) {
    return core::bounded_string(buf, capacity, reported);
}
