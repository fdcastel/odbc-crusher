...
| **#317 / #316** implementation descriptors and `SQLColAttribute` | ❌ | ✅ ×3 | `P6`/`P7`. `IRD concise=SQL_C_DEFAULT(99) - an unfilled record, name=''` while `SQLDescribeCol` on the same statement answers `SQL_INTEGER`/`ID`; and `SQL_DESC_LENGTH=9187201948296675328` — `0x7F7F7F7F00000000`, the guard pattern left in the upper half of a `SQLLEN` the driver wrote 32 bits of . And, since **P19** corrected it, `SQL_DESC_DATETIME_INTERVAL_CODE rejected (HY091)` on a **TIMESTAMP** column against `=3` on the patched build — the same detection, resting on a field the specification defines for that column rather than one it leaves undefined |
...
| `P18` | **Three probe bugs, two of them reporting a conformant driver as defective** — found by running the whole fleet, which the fourteen new probes had never met. P13 *returned* on a non-canonical `SQL_DBMS_VER` and so never ran the check it exists for; its tail scanner read `0.24` out of MySQL's `ubuntu0.24`; P4 graded MariaDB's `HY024` refusal of `SQL_ATTR_KEYSET_SIZE` as a defect |
| `P19` | **Two descriptor probes graded what the specification leaves undefined** — `SQL_DESC_NAME` on an expression column (documented empty; MariaDB was right and the probe was wrong) and `SQL_DESC_DATETIME_INTERVAL_CODE` on an INTEGER column (undefined there). All four non-Firebird drivers failed the second one on that alone. Fixing it uncovered a mock `SQLGetDescFieldW` that returned ANSI bytes to a UTF-16 caller — **which the probe could not detect, because it only checked the name for emptiness and garbage is not empty** |
