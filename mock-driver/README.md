...
| `FailOn` | Function names | Comma-separated functions to fail, in any mode including the default `Success`. Naming a function also narrows `Failure`/`Random` to the named ones. **A `…W` name** (D78) fails only calls that entered through that Unicode entry point; an ANSI name fails both widths. Through a driver manager this driver receives both widths as W, so the W names are a lever for these tests rather than something an application can distinguish. |
