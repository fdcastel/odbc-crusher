...
SQLRETURN SQL_API SQLGetDiagRec(
    ...
    // D36: FailOn reaches the diagnostic readers too - A23 and the
    // error-queue probes had no way to be proven wrong without it. No
    // diagnostic is posted here: a reader that fails must not add to the
    // queue it was asked to read.
    {
        const auto& fi_config = BehaviorController::instance().config();
        if (fi_config.should_fail("SQLGetDiagRec")) return SQL_ERROR;
    }
...
SQLRETURN SQL_API SQLGetDiagField(
    ...
    {
        const auto& fi_config = BehaviorController::instance().config();
        if (fi_config.should_fail("SQLGetDiagField")) return SQL_ERROR;
    }
